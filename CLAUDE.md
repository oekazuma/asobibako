...
らくがきパレード（`doodle-worm`）は、描きためた線を「うごけ！」で 1 匹にまとめる（`engine.ts` の `hatch`）。いちばん大きな輪が体で、ほかの線は半分以上が体の中なら顔（顔があれば目を足さない）、外なら体の中心から見た向きで足・はね・触角・しっぽ（片側だけに長く伸びた線）になる。体から離れた線や下へ長く伸びた線（棒人間の胴や足）は振るとつないだ線が外れて見えるので `still` として体といっしょに動かすだけにする。動き方は手足で決まり、しっぽなら這い、足なら歩き、はねなら飛び、何もなければ跳ねる。輪のない絵は全体をくねらせて這う。
