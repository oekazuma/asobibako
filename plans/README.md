...
| 037  | 外したタップの点だけの子を、ずかんに入れない                                                    | P1       | S      | —          | DONE（`e5de739`、レビュー済み）                                                  |
| 038  | ペットに「うちに来た日」を持たせ、何日めかを札に出す                                            | P2       | S      | —          | DONE（`137b41b`、レビュー済み）                                                  |
| 039  | 音が止まっているあいだの足音や物音を溜めない                                                    | P3       | S      | —          | DONE（`ba4fe5d`、レビュー済み）                                                  |
| 040  | ひらめきナゾを好きな順に解けるようにし、解いたナゾを 1 問ずつ覚える                             | P1       | M      | —          | DONE（`fedf466`、レビュー済み）                                                  |
| 041  | 場面に入るとき、シェーダーの準備が済むまで描かずに待つ                                          | P3       | S      | —          | DONE（`745fc38`、レビュー済み）                                                  |
| 042  | 初めて組み立てるペットを 1 フレームに 1 匹ずつにし、長く固まらないようにする                    | P2       | M      | 041        | DONE（`d02cf42`、041 を含む、40ms ずつに直させた、レビュー済み）                 |

| 043 | ペットの毛並み（毛の殻）と、3D の画質の設定をなくす | P1 | M | — | DONE（`837dc44`、レビュー済み。見た目は headless Chrome で前後を比べた） |

| 044 | バックアップの赤い注意書きと「？」の印をなくし、「この端末の控え」を分かる言葉にする | P1 | S | — | DONE（`df7119c`、レビュー済み） |
...
- 041・042 の効果を headless Chrome で測り直した。控えなしで初めてひろばに入るときの止まりは 2.4 秒の 1 回から約 0.3 秒 ×9 回に
  分かれ（CPU ×4 で 9.9 秒の 1 回から約 1.2 秒 ×9〜10 回）、控えありのひろば入りは長いタスクが 0 になった。覆いが外れた直後の
  画面は 6 枚とも描けていた。残りは、部屋を開くときの最初の描画で準備される影のシェーダー（`compileAsync` は影の深度用の材質を
  準備しない見込み、約 20ms）と、組み立て待ちのあいだに新しい材質をその場で準備して 2 匹ぶん止まる回（約 0.5 秒）
- 043 のあと、三毛猫のしっぽの先に灰色の輪が見える。体の面に穴は無い（15 種類とも 1 枚の三角形にしか使われない辺は 0 本）。
  前から毛の殻の下にうっすら見えていたもので、黒っぽい毛の輪郭に足す青みのつや（`fur.ts` の `furDark` の emissive）が、
  殻が無くなってくっきり出たとみられる。気になるなら、殻の無い体ではこのつやを弱める
