...
- 絵柄はちいかわ・たまごっち寄りのゆるかわ。線は `#5b4a42` の細い 1 本、影とつやなし、目は小さな黒丸、ほおは「///」、耳は顔にくっつける。見本は `.superpowers/sdd/2026-09-23-dentist/kawaii-v3.html`、配置は `.superpowers/brainstorm/24986-1790137317/content/layout-v2.html`（A）
- 患者さんは うさぎ・ねこ・いぬ・ぶた・くま・ライオン・カバ・カエル（`AnimalId` は `frog`）。片側の歯の本数 `per` は rabbit 3、cat 4、dog 4、bear 5、pig 5、lion 5、hippo 6、frog 6。歯の形は `round`（うさぎだけ `incisor`）で、`fang` は使わない
...
- [ ] Step 7. **ここで止まり、利用者にスクリーンショットを見せて確認をもらう**（仕様の「進め方」）。サブエージェントで実行しているときは、Step 6 までで止めて確認ページのパスとスクリーンショットをコントローラーに返し、利用者への確認はコントローラーが行う。直してほしいと言われたら直して撮り直し、了承が出るまで次へ進まない
...
### Task 2R: ゆるかわへの描き直しとカエル（利用者の確認が要る）

Task 1・2 のあと、利用者から「怖い・海外っぽい。ちいかわやたまごっちのようなゆるかわに」「ワニはカエルに」「耳を顔にくっつけて自然に」と指示があった。
見本 `.superpowers/sdd/2026-09-23-dentist/kawaii-v3.html`（SVG）の絵柄に、Task 1・2 の成果を合わせて直す。

**Files:**

- Modify: `src/lib/games/dentist/animals.ts`、`animals.test.ts`
- Modify: `src/lib/games/dentist/paint/style.ts`、`patient.ts`、`germ.ts`、`teeth.ts`
- 一時的（コミットしない）: `src/routes/dev-dentist/+page.svelte`

**Interfaces:**

- 変えないもの: `drawPatient(ctx, a, expr, t, teeth)`、`mouthPath(a)`、`drawGerm(ctx, x, y, kind, t, look)`、`toothPath(t)`、`drawToothBase(ctx, t)`、`LINE`、`LW`、`ink`、`oval`、`teethFor`、`FACE`、`GERM_R`、`Expression`
- 変えるもの: `AnimalId` の `croc` を `frog` に。`ToothShape` から `fang` を外す。`Animal` の `ears` / `extra` は描き直しに合わせて変えてよい

- [ ] Step 1. `animals.ts` を直す。`per` は Global Constraints の値。口（`mouth`）は顔の中に収まる「あーん」の大きさにし、上下の列が重ならないこと（`animals.test.ts` のテスト）を満たす範囲で、歯をタップしやすいよう大きめにとる。`animals.test.ts` の本数のテストを「うさぎの 6 本からカエルの 12 本まで」に直す
- [ ] Step 2. `paint/` を見本のタッチで描き直す。線は `LINE = '#5b4a42'` の細い 1 本、影とつやは入れない。顔はおもちのような丸い形。目は小さな黒丸に白い光、ほおはピンクの丸に「///」。耳は顔の輪郭に重ねて付け根を顔の後ろに隠し、離れて見えないようにする。口の中は明るいピンク（`#f7a1ae` 前後）、のど・舌も明るく。歯は白く丸い。表情 5 つとバイキン 3 種も見本に合わせる
- [ ] Step 3. 確認ページ `http://localhost:5173/table-duel/dev-dentist` で見て直す（built-in browser だけを使う）。確認ページはコミットしない
- [ ] Step 4. `pnpm exec vitest run src/lib/games/dentist`、`pnpm check`、`pnpm exec eslint src/lib/games/dentist` を通してコミットする
- [ ] Step 5. **利用者に確認をもらう**（コントローラーが行う）。了承が出たら確認ページを消す
...
- [ ] Step 1. アイコンを足す。`icons.ts` の先頭の定数の近くに `const LINE = '#5b4a42';` を置き、`ICONS` に次を足す（名前は `ToolId` とそろえる。`bear` はすでにある）
...
- [ ] Step 2. **利用者にスクリーンショットを見せ、市販の子ども向けアプリと比べて見劣りするところがないか確認をもらう**（利用者とのやりとりはコントローラーが行う）。指摘があれば直して撮り直す
...
| ゆるかわの絵・表情 5 つ・バイキン 3 種・道具の演出・効果音       | 2・8                                                                             |
| キャラクター表の確認                                             | 2（Step 7）                                                                      |
