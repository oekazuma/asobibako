<script lang="ts">
  ...
  import { backupFile, backupName, exportAll, importAll, parseBackup, summarize, type Backup } from '$lib/backup';
  ...
  function share() {
    ...
    navigator.share({ files: [backupFile(version)], title: 'あそびばこ の記録' }).catch((e: unknown) => {
