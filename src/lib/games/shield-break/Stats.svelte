...
<div class="stats">
  <span class="row" role="img" aria-label="体力 {game.life[player]}">
    {#each count(LIFE) as i (i)}<span class="heart" class:off={i >= game.life[player]}><Icon name="heart" /></span
      >{/each}
  </span>
  <span class="row" role="img" aria-label="エネルギー {game.energy[player]}">
    {#each count(MAX_ENERGY) as i (i)}<span class="pip energy" class:on={i < game.energy[player]}
        ><Icon name="bolt" /></span
      >{/each}
  </span>
  <span class="row" role="img" aria-label="盾 {game.shield[player]}">
    {#each count(SHIELD) as i (i)}<span class="pip shield" class:on={i < game.shield[player]}
