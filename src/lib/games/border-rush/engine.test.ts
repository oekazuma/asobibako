...
import { createState, GAIN, pop, spawnOrb, zone } from './engine';
...
describe('border-rush engine', () => {
	...
	it('相手の玉は取れない', () => {
		const s = createState();
		const orb = spawnOrb(s, 'tap', 1, seq(0.5));
	...
	it('境界線上の玉はどちらでも取れる', () => {
		const s = createState();
		const orb = spawnOrb(s, 'contest', null, seq(0.5));
	...
	it('境界が動いて相手側に取り残された玉は消える', () => {
		const s = createState();
		const deep = spawnOrb(s, 'tap', 2, seq(0)); // 陣地のいちばん上端寄り
		const edge = spawnOrb(s, 'tap', 2, seq(1)); // 境界のすぐ上
		const mine = spawnOrb(s, 'hold', 1, seq(1));
